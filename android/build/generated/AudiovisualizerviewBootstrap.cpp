...
#define TAG "ti.audiovisualizerview"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Audiovisualizerview_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Audiovisualizerview.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::AudiovisualizerviewBindings::lookupGeneratedInit(
...
static void Audiovisualizerview_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Audiovisualizerview_getBinding)->GetFunction());
}

static void Audiovisualizerview_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::AudiovisualizerviewBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AudiovisualizerviewBinding = {
	"ti.audiovisualizerview",
	Audiovisualizerview_init,
	Audiovisualizerview_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_audiovisualizerview_AudiovisualizerviewBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.audiovisualizerview", &AudiovisualizerviewBinding);
	titanium::KrollBindings::addExternalLookup(&(::AudiovisualizerviewBindings::lookupGeneratedInit));
}
