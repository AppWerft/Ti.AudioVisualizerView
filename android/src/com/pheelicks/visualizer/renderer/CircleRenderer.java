...
public class CircleRenderer extends Renderer {
	...
	public void onRender(Canvas canvas, AudioData data, Rect rect) {
		...
		for (int i = 0; i < data.bytes.length - 1; i++) {
			float[] cartPoint = {
					...
					rect.height() / 2 + ((byte) (data.bytes[i] + 128))
							* (rect.height() / 2) / 128 };
			...
			float[] cartPoint2 = {
					...
					rect.height() / 2 + ((byte) (data.bytes[i + 1] + 128))
							* (rect.height() / 2) / 128 };
	...
	private float[] toPolar(float[] cartesian, Rect rect) {
		...
		double radius = ((rect.width() / 2) * (1 - aggresive) + aggresive
				* cartesian[1] / 2)
				* (1.2 + Math.sin(modulation)) / 2.2;
		float[] out = { (float) (cX + radius * Math.sin(angle)),
